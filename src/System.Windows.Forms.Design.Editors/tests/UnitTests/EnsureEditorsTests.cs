...
namespace System.Windows.Forms.Design.Editors.Tests
{
    public class EnsureEditorsTests
    {
        ...
        {
            var reflectType = TypeDescriptor.GetReflectionType(type);
            Assert.NotNull(reflectType);

            var method = reflectType.GetMethod(methodName);
            Assert.NotNull(method);

            var attributes = method.GetCustomAttributes(typeof(EditorAttribute), false);
